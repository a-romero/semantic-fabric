...
| **`fabric_answer` (MCP) times out but a plain `fabric_search` is instant** | `fabric_answer` also runs a 2-hop GraphRAG expansion (`/graph/expand`). On a large, densely-connected graph a hub entity (mentioned by hundreds of pages) makes the walk fan out enormously. The traversal is now **bounded** (`GRAPH_EXPAND_MAX_NODES`, default 250) so it stays in the millisecond range; the returned top-k related pages are unchanged. Raise the cap for a wider neighbourhood, lower it to bound worst-case latency further. |
